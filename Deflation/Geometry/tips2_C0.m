function [output, square_deg] = tips2_C0(eps_vec, d, ref)

% TIPS2_C0: Geometry with 2 tips for C^0 discretizations.
% INPUT:
...
for k = 1:length(eps_vec)
    ...
    shift = 1/32;
    % shift = 1/25;
    A = [2*h+shift, 0]; B = [2*h+shift, 0.5]; C = [7*h+shift, 12*h+eps]; D = [8*h, 11*h];
    F = [8*h+shift 12*h+eps]; G = [14*h-shift 0.5]; H = [14*h-shift 0];

    loop_0 = struct();
    loop_0(1).curve = nrbline(A, B);
    loop_0(2).curve = nrbline(B, C);
    loop_0(3).curve = nrbline(C, D);
    loop_0(4).curve = nrbline(D, F);
    loop_0(5).curve = nrbline(F, G);
    loop_0(6).curve = nrbline(G, H);
    loop_0(7).curve = nrbline(H, A);
    loop_0(1).label = 5;
    loop_0(2).label = 6;
    loop_0(3).label = 7;
    loop_0(4).label = 8;
