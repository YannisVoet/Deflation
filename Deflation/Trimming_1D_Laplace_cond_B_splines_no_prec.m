...
% Conditioning of the non-preconditioned matrices for the B-spline basis.
...
precond_tech = {'No preconditioning'};
...
c=colorgrad([0.8 0.8 0.8], [0 0 0], nd);
...
for k=1:n_prec
    ...
    for j=1:nd
        trendline(eta_vec(1:15), T(k).cond_K(1:15,j), color=c(j,:), linestyle='-', linewidth=0.5, variable='\eta', subset=true)
    end
end
