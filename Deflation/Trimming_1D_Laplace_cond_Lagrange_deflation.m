...
% Conditioning of the deflation preconditioned matrices for the Lagrange 
% basis and various values of gamma.
...
precond_tech = {'No preconditioning', 'Jacobi', 'Deflation', 'Deflation', 'Deflation'};
...
% volume threshold for identifying elements with small volume ratio
gamma=[0.1 0.25 1];
n_gamma=length(gamma);

c1=[255, 192, 203]/255; % light pink
c2=[0.9 0.1 0.1];       % dark red
[c]=colorgrad(c1,c2,n_gamma);
c=num2cell(c,2);

addendum=arrayfun(@(x) [' ($\gamma=' num2str(x) '$)'], gamma, 'UniformOutput', false);

param=struct('gamma', num2cell(gamma));
param=num2cell(param, 1);

[T(id_defl).color]=c{:};
[T(id_defl).addendum]=addendum{:};
[T(id_defl).param]=param{:};
...
for d = drange
    ...
    figure
    for k=1:n_prec
        loglog(eta_vec, T(k).cond_M, 'Linestyle', 'none', 'Marker', T(k).marker, 'Color', T(k).color, 'LineWidth', 1, 'DisplayName', [T(k).name T(k).addendum])
        hold on; grid on;
    end
    ...
    figure
    for k=1:n_prec
        loglog(eta_vec, T(k).cond_K, 'Linestyle', 'none', 'Marker', T(k).marker, 'Color', T(k).color, 'LineWidth', 1, 'DisplayName', [T(k).name T(k).addendum])
        hold on; grid on;
    end
