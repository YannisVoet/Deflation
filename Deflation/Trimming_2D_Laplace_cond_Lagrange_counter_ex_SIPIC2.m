% Second counter-example for SIPIC with the Lagrange basis.
...
precond_tech = {'No preconditioning', 'Jacobi', 'SIPIC'};
...
trendline(eta_vec, T(id_sip).cond_M, color=T(id_sip).color, linestyle='-', linewidth=0.5, variable='\eta', subset=true)
...
trendline(eta_vec, T(id_sip).cond_K, color=T(id_sip).color, linestyle='-', linewidth=0.5, variable='\eta', subset=true)
