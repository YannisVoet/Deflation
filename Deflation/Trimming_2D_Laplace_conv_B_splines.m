...
% for the B-spline basis for various preconditioning strategies and various 
% trimmed geometries.
...
param_defl=struct('reduct', reduct_val, 'string', reduct_cell);

T(end-1).param=param_defl(1);
T(end).param=param_defl(2);

[T(id_defl).addendum]=reduct_cell{:};
...
title('Relative residual')
legend(h, strcat({T.name}, {T.addendum}), 'Location', 'best')
legend show
xlabel('Iteration number')
ylabel('Relative residual')
...
figure
for k=2:n_prec
    semilogy(T(k).e(1:ns), 'Linestyle', 'none', 'Marker', T(k).marker, 'Color',  T(k).color, 'LineWidth', 1, 'DisplayName', [precond_tech{k} T(k).addendum])
    hold on; grid on;
end
title('Spectrum of $M$')
...
sgtitle('Spectrum of $M$', 'Interpreter', 'Latex')
